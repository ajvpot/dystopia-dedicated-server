...
if [ ! -f "${GAME_DIR}/.installed" ]; then
  update
  wget -qO- "https://mms.alliedmods.net/mmsdrop/1.11/mmsource-1.11.0-git${METAMOD_VERSION}-linux.tar.gz" | tar xvzf - -C "${GAME_DIR}/dystopia" &&
    rm "${GAME_DIR}/addons/metamod_x64.vdf"
  # Install Sourcemod (comment out for tournament servers)
  wget -qO- "https://sm.alliedmods.net/smdrop/1.10/sourcemod-1.10.0-git${SOURCEMOD_VERSION}-linux.tar.gz" | tar xvzf - -C "${GAME_DIR}/dystopia"
